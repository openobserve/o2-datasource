...
#######################################
# CloudWatch Logs to OpenObserve Cleanup Script
...
show_banner() {
    echo -e "${CYAN}"
    echo "================================================"
    echo "   CloudWatch Logs to OpenObserve Cleanup"
    echo "================================================"
    echo -e "${NC}"
}
...
find_stacks() {
    ...
    # Search for both new naming (cw-logs-*) and old naming (cloudwatch-logs*)
    STACKS=$(aws cloudformation list-stacks \
        --profile "$AWS_PROFILE" \
        --region "$REGION" \
        --stack-status-filter CREATE_COMPLETE UPDATE_COMPLETE UPDATE_ROLLBACK_COMPLETE \
        --query 'StackSummaries[?starts_with(StackName, `cw-logs-`) || contains(StackName, `cloudwatch-logs`)].StackName' \
        --output text)
...
main() {
    ...
    LAMBDA_LOGS=$(aws logs describe-log-groups \
        --profile "$AWS_PROFILE" \
        --region "$REGION" \
        --log-group-name-prefix "/aws/lambda/cloudwatch-logs" \
