...
#######################################
# Route53 Query Logs to OpenObserve Cleanup Script
...
REGION="us-east-1"  # Route53 is a global service, resources must be in us-east-1
...
show_banner() {
    echo -e "${CYAN}"
    echo "================================================"
    echo "   Route53 Query Logs to OpenObserve Cleanup"
    echo "================================================"
    echo -e "${NC}"
}
...
find_stacks() {
    ...
    # Search for Route53 stacks (must be in us-east-1)
    STACKS=$(aws cloudformation list-stacks \
        --profile "$AWS_PROFILE" \
        --region "$REGION" \
        --stack-status-filter CREATE_COMPLETE UPDATE_COMPLETE UPDATE_ROLLBACK_COMPLETE \
        --query 'StackSummaries[?starts_with(StackName, `route53-`)].StackName' \
        --output text)

    if [ -z "$STACKS" ]; then
        print_warning "No Route53 stacks found in $REGION" >&2
        exit 0
    fi
...
# Remove Route53 Query Logging Configuration
remove_query_logging_config() {
    local STACK_NAME=$1
    local REGION=$2

    print_header "Removing Route53 Query Logging Configuration"

    # Get hosted zone ID from stack name (route53-Z1234567890ABC)
    HOSTED_ZONE_ID=$(echo "$STACK_NAME" | sed 's/route53-//')

    if [ -n "$HOSTED_ZONE_ID" ]; then
        print_info "Checking query logging config for hosted zone: $HOSTED_ZONE_ID"

        # List query logging configs for this hosted zone
        CONFIGS=$(aws route53 list-query-logging-configs \
            --profile "$AWS_PROFILE" \
        ...
        if [ -n "$CONFIGS" ]; then
            for config_id in $CONFIGS; do
                print_info "Deleting query logging config: $config_id"
                aws route53 delete-query-logging-config \
                    --profile "$AWS_PROFILE" \
                    --id "$config_id" 2>/dev/null || print_warning "Failed to delete config (may already be deleted)"
            done
            print_success "Query logging configurations removed"
        else
            print_info "No query logging configurations found"
        fi
    else
        print_info "Could not extract hosted zone ID from stack name"
    fi
}
...
show_summary() {
    ...
    # Extract hosted zone ID from stack name
    HOSTED_ZONE_ID=$(echo "$STACK_NAME" | sed 's/route53-//')
...
main() {
    ...
    if [ ${#STACK_ARRAY[@]} -gt 1 ]; then
        ...
        for i in "${!STACK_ARRAY[@]}"; do
            # Extract hosted zone ID for display
            ZONE_ID=$(echo "${STACK_ARRAY[$i]}" | sed 's/route53-//')
            ...
            echo "  $((i+1))) ${STACK_ARRAY[$i]} - Zone: $ZONE_NAME"
        done
    ...
    for STACK_NAME in "${SELECTED_STACKS[@]}"; do
        ...
        # Remove query logging configuration
        remove_query_logging_config "$STACK_NAME" "$REGION"
    ...
    LAMBDA_LOGS=$(aws logs describe-log-groups \
        --profile "$AWS_PROFILE" \
        --region "$REGION" \
        --log-group-name-prefix "/aws/lambda/route53-" \
    ...
    # Check for orphaned Route53 log groups
    print_info "Checking for orphaned Route53 log groups..."
    ROUTE53_LOGS=$(aws logs describe-log-groups \
        --profile "$AWS_PROFILE" \
        --region "$REGION" \
    ...
    if [ -n "$ROUTE53_LOGS" ]; then
        print_warning "Found orphaned Route53 log groups: $ROUTE53_LOGS"
        print_info "You may want to delete these manually if no longer needed"
    fi
}
